...
def test_suggest_pause_with_env_flag_actually_pauses(workspace, monkeypatch):
    impl = workspace["memory_mcp.impl"]
    ...
    paused_only = impl.get_bt_leaderboard(top_k=10, include_paused=False)
    assert all(row["node_id"] != b for row in paused_only)
    full = impl.get_bt_leaderboard(top_k=10, include_paused=True)
    by_id = {row["node_id"]: row for row in full}
...
def test_auto_prune_env_variations(workspace, monkeypatch):
    ...
    for falsy in ("", "0", "false", "False"):
        ...
        result = impl.suggest_pause_low_strength(
            ucb_threshold=10.0, min_comparisons=4
        )
