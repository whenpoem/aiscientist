...
**v4.0.0a0 (proof trunk, alpha) is shipped.** The `prove_mcp` MCP server, the `prove-sop` skill, the `prover` agent definition, the cold-start seed scripts in `scripts/`, and the reviewer dual checklist all landed. Lean reinsurance remains opt-in: `_lean` in `.claude/settings.json` is disabled until the user manually installs elan + mathlib + lean-lsp-mcp per [`docs/setup-lean.md`](docs/setup-lean.md). See [ADR 0008](docs/adr/0008-two-trunk-domain-architecture.md) for the architecture, [ADR 0007](docs/adr/0007-tools-skills-hooks-layering.md) for the layering doctrine that gates new capability, and [architecture.md §13](docs/architecture.md#13-core-vs-domain-trunks-v40) for the core/trunk split.
