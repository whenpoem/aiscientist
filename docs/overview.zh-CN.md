...
假设你在终端 A 输入：`/research-sop 研究 dropout 对 ViT 是否有影响`
...
这样做的好处：
- 整个系统状态可以通过复制一个文件完成备份
- 跨模块的原子操作天然就是一个 SQL 事务
...
1. **决策阶段**：生成假说 → BT 锦标赛排名 → 预注册指标和阈值
2. **实验阶段**：预算门禁 → 安全检查 → 跑实验 → 多种子稳定性验证 → 公平性比较 → 溯源记录
3. **写作阶段**：reviewer 审稿，逐条核对每个数字是否有 pin、有稳定 seed、有 met 的预注册、有未漂移的 provenance

写作阶段的 reviewer 会拒绝任何无法回溯到上述四要素的数值声明，这就是项目对"可信结果"的工程化保证。
...
- **不是一个完整的 AI Scientist 替代品**。它增强 Claude Code，但不取代你的研究判断。
- **不是浏览器应用**。Cockpit 是终端 TUI，不需要启动 Vite、不需要 uvicorn、不占用任何端口。这是 v0.2 中刻意做出的简化决定。
- **不是多用户系统**。当前设计假定单用户单会话；多会话并发的乐观锁机制还在路线图上。
- **不是已经准备好对外宣称"production-ready"的产品**。108 个测试通过且 ruff 干净，但生产就绪需要一次完整的端到端验证。
